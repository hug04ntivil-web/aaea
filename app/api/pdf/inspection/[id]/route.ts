...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await new Promise<void>(resolve => {
    ...
    // ── Leyenda de cálculo de nota (centrada bajo los score boxes) ───────────
    {
      const legendW = BW + 3 * SW + 3 * SGAP   // mismo ancho que los score boxes = 308
      const legendX = ML + Math.round((CW - legendW) / 2)
      doc.fillColor(C.muted).font("Helvetica").fontSize(6)
        .text("* Nota calculada sobre ítems evaluados. Se excluyen los marcados como N/A (No Aplica).", legendX, y, { width: legendW, align: "center" })
    }
    y += 14
